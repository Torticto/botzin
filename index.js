    ...
    client.onAddedToGroup(({ groupMetadata: { id }, contact: { name } }) =>
        client.getGroupMembersId(id)
            .then((ids) => {
                ...
                if (ids.length <= 1) {
                    client.sendText(id, `Obrigado por me colocar neste grupo ;)\nDigite ${prefix}menu para ver meus comandos.`).then(() => client.leaveGroup(id))
                } else {
                    client.sendText(id, `Obrigado por me colocar neste grupo ;)\nDigite ${prefix}menu para ver meus comandos.`)  
                }
            }))
